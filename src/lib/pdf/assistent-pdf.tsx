...
export type AssistentPDFData = {
  ...
  city:           string | null
  iban:           string | null
  shifts:         AssistentPDFShift[]
  invoiceNumber?: string
  invoiceDate?:   string
  sender:         SenderInfo
}

export function AssistentDocument({ data }: { data: AssistentPDFData }) {
      ...
      <Page size="A4" style={s.page} orientation="landscape">
        ...
        <View style={s.titleBar}>
          <Text style={s.docTitle}>
            {data.invoiceNumber ? `FACTUUR ${data.invoiceNumber}` : 'Overzicht Gewerkte Uren'}
          </Text>
        ...
        <View style={s.parties}>
          ...
          <View style={s.partyBox}>
            ...
            <Text style={s.partyName}>{data.firstName} {data.lastName}</Text>
            {data.companyName && <Text style={s.partyLine}>{data.companyName}</Text>}
            ...
            {data.iban       && <Text style={s.partyMuted}>IBAN: {data.iban}</Text>}
          </View>
        </View>
        ...
        {/* Month banner */}
        <View style={s.banner}>
          <Text style={s.bannerTitle}>Prestaties — {fmtMonth(data.month)}</Text>
        ...
        <View style={s.table}>
          <View style={s.thead}>
            ...
            <Text style={[s.th, s.colPharmacy]}>Apotheek</Text>
          ...
          {data.shifts.map((shift, i) => {
            ...
            return (
              <View key={shift.id} style={Row}>
                ...
                <Text style={[s.td,     s.colPharmacy]}>{shift.pharmacyName}</Text>
        ...
        <View style={s.totalsWrap}>
          <View style={s.totalsBox}>
            ...
            <View style={s.totRow}>
              <Text style={s.totLabel}>BTW {data.vatLiable ? '21%' : '0%'}</Text>
