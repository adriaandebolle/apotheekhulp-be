...
type SortField = 'company_name' | 'email'
...
function SortHeader({ label, field, sort, dir, q }: { label: string; field: SortField; sort: string; dir: string; q: string }) {
  ...
  return (
    <Link href={`/admin/gebruikers/apotheken?${params}`} className="inline-flex items-center gap-1 hover:text-text group">
      {label}
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-xs text-text-muted mb-1">Overzicht Gebruikers / Apotheken</p>
          <h1 className="text-2xl font-bold text-text">Apotheken</h1>
        </div>
        <ButtonLink href="/admin/gebruikers/apotheken/nieuw" size="sm">
          + Nieuwe apotheek
        </ButtonLink>
      </div>
      ...
      <div className="flex gap-0 mb-6 border-b border-border">
        <Link href="/admin/gebruikers/assistenten" className="px-4 py-2 text-sm font-medium text-text-muted hover:text-text transition-colors">Assistenten</Link>
        <Link href="/admin/gebruikers/apotheken"   className="px-4 py-2 text-sm font-medium text-primary border-b-2 border-primary -mb-px">Apotheken</Link>
      </div>

      {/* Stats + Search */}
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-text-muted">Actieve apotheken <strong className="text-text">{activeCount}</strong></p>
        <SearchInput
          defaultValue={q}
          placeholder="Zoeken..."
          basePath="/admin/gebruikers/apotheken"
      ...
      <Table>
        <Thead>
          <Tr>
            <Th><SortHeader label="Apotheeknaam" field="company_name" sort={sort} dir={dir} q={q} /></Th>
        ...
        <Tbody>
          {apotheken.length === 0 && <EmptyRow colSpan={5} />}
          {apotheken.map(a => {
            const name = a.company_name || a.email
            return (
              <Tr key={a.id}>
                ...
                <Td className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    {/* Edit */}
                    <ButtonLink href={`/admin/gebruikers/apotheken/${a.id}`} variant="ghost" size="sm" title="Bewerken">
                    ...
                    <form action={startImpersonation}>
                      ...
                      <button type="submit" title="Aanmelden als deze apotheek"
