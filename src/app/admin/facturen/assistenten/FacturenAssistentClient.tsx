...
import type { AssistentUninvoicedRow, InvoiceRow } from './page'
...
function InvoiceActions({ inv }: { inv: InvoiceRow }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <a
        href={`/api/pdf/assistent?invoice_id=${inv.id}`}
        target="_blank"
...
export default function FacturenAssistentClient({
  uninvoicedRows,
  invoiceRows,
  month,
  months,
}: {
  uninvoicedRows: AssistentUninvoicedRow[]
  ...
  const [modalRow, setModalRow] = useState<AssistentUninvoicedRow | null>(null)
    ...
    <>
              ...
              {uninvoicedRows.length === 0
                ...
                : uninvoicedRows.map(row => (
                  <Tr key={row.id}>
                    ...
                    <Td className="text-text-muted">{row.pharmacyNames.join(', ') || '—'}</Td>
      ...
      {modalRow && (
        <CreateInvoiceModal
          ...
          recipientName={`${modalRow.firstName} ${modalRow.lastName}`}
