...
export function InstellingenForm({ config }: { config: PlatformConfig }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updatePlatformConfig({
        ...
        default_hourly_rate_assistant: parseFloat(fd.get('default_hourly_rate_assistant') as string),
        default_hourly_rate_pharmacy:  parseFloat(fd.get('default_hourly_rate_pharmacy')  as string),
      })
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-xl p-6 space-y-6">
      <div>
        <Label htmlFor="km_rate" required>Kilometervergoeding (€/km)</Label>
        <Input
          id="km_rate"
          name="km_rate"
          type="number"
        ...
        <p className="mt-1 text-xs text-text-muted">
          Wettelijk tarief 2024–2025: €0,4296/km. Wordt gebruikt als standaard bij nieuwe koppelingen.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label htmlFor="default_hourly_rate_assistant" required>Standaard uurtarief assistent (€)</Label>
          <Input
        ...
        <div>
          <Label htmlFor="default_hourly_rate_pharmacy" required>Standaard uurtarief apotheek (€)</Label>
          <Input
      ...
      <div>
        <Label htmlFor="vat_rate" required>BTW-percentage (%)</Label>
        <Input
          id="vat_rate"
          name="vat_rate"
          type="number"
        ...
        />
        <p className="mt-1 text-xs text-text-muted">
          Huidige BTW voor diensten: 21%. Enkel aanpassen als het wettelijk tarief wijzigt.
        </p>
      </div>

      {feedback.error   && <p className="text-sm text-danger bg-red-50 border border-red-200 rounded-lg px-3 py-2">{feedback.error}</p>}
      {feedback.success && <p className="text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2">{feedback.success}</p>}
